...
contract UniswapPayoutSpikeTest is Test {
    ...
    string private constant FIXTURE_PATH = "test/fork/fixtures/uniswap-route.json";
    ...
    ///      always intends to execute it: a missing RPC or fixture is an
    ///      operator error and must fail loudly. A test that returns green
    ///      having asserted nothing is worse than no test.
    function setUp() public {
        string memory fixture = vm.readFile(FIXTURE_PATH);
        require(
            bytes(fixture).length != 0,
            "fork spike: fixture is empty; run `node frontend/scripts/fetch-uniswap-route.mjs`"
        );
    ...
    modifier requiresFixture() {
        require(fixtureLoaded, "fork spike: fixture not loaded");
        _;
    }
